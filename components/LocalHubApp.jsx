...
function VendorDash({user,country,rates,products,setProducts,vendorOrders,updateVendorOrder,promos,setPromos,fire}){
  ...
  const saveProduct=async()=>{if(!form.name||!form.price)return;const category=(form.cat||"").trim()||"General";const p={name:form.name.trim(),price:+form.price,emoji:form.emoji,cat:category,sizes:form.sizes.split(",").map(s=>s.trim()).filter(Boolean),stock:+form.stock||0,color:form.color,imageUrl:form.imageUrl,country,vendorId:user.id,reviews:[],sales:0,revenue:0};if(editItem==="new"){let created={...p,id:Date.now()};if(_accessToken){try{const data=await API.createProduct({...p,category});if(data?.product)created={...normalizeProduct(data.product),vendorId:user.id};}catch(e){fire(e.message||"Product saved locally, but backend create failed.");}}setProducts(ps=>[created,...ps]);fire("Product added successfully.");}else{setProducts(ps=>ps.map(x=>x.id===editItem.id?{...x,...p}:x));fire("Product updated successfully.");}setEdit(null);};
