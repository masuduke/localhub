...
export function setRefreshCookie(res, token) {
  ...
  res.setHeader("Set-Cookie", [
    `refreshToken=${token}; HttpOnly; ${secureAttr}SameSite=Strict; Path=/api/auth/refresh; Max-Age=${REFRESH_EXPIRES_MS / 1000}`,
  ]);
}
...
export function clearRefreshCookie(res) {
  const secureAttr = process.env.NODE_ENV === "production" ? "Secure; " : "";
  res.setHeader("Set-Cookie", [
    `refreshToken=; HttpOnly; ${secureAttr}SameSite=Strict; Path=/api/auth/refresh; Max-Age=0`,
  ]);
}
